...
export function CardEditorFields(props: Props) {
  ...
  return <>
    <section className="editor-section">
      <div className="section-title"><div><h2>题目材料</h2></div></div>
      <div className="asset-editor">{assets.map((asset) => <div className="asset-preview" key={asset.id}>
        ...
        <button className={`upload-zone${draggingImage ? " dragging" : ""}`} type="button"
          ...
          onDrop={acceptDrop}><ImagePlus size={25} /><strong>添加图片</strong>
          </button>
      ...
      <label className="field full"><span>题目</span><textarea {...form.register("question")} rows={4} /></label>
    </section>
    <section className="editor-section">
      <div className="section-title"><div><h2>作答与诊断</h2></div></div>
      ...
      <details className="editor-more"><summary>解法与补充记录</summary><div className="form-grid editor-explanation"><label className="field full"><span>正确解法</span>
        <textarea {...form.register("solution")} rows={4} /></label>
        <label className="field full"><span>补充说明 <small>可选</small></span><textarea {...form.register("supplementalNote")} rows={2} /></label></div></details>
    </section>
    <section className="editor-section">
      <div className="section-title"><div><h2>知识归档</h2></div></div>
