...
> 2026-09-05 升级进展：用户已确认[升级计划书 C01～C05](./项目升级从需求到落地计划书.md)，相关旧规则进入修订：业务页直接完成明确任务；Agent 具备所有类型卡片完整编辑和联网搜索能力；错题先应用建议再保存；练习在来源与参数页一次“生成并保存”。本轮范围为 S1～S3，知识卡深度改造后置。下文与这些决定冲突的“Agent 必经入口、独立练习计划批准”等内容不再作为新增实现依据；其他未受影响约束继续保留。候选[简化原型](./demo/learning-flow-upgrade/index.html)和[差异说明](./demo/learning-flow-upgrade/README.md)待 C06 评审，本文后续再统一修订版本。
