...
function review(state) {
  ...
  return `<div class="review-toolbar"><button type="button" data-action="previous-exercise" ${state.flipIndex===0?"disabled":""}>上一题</button><div><strong>${state.flipIndex+1} / ${exercises.length}</strong>${exercises.map((_,index)=>`<button type="button" data-jump-exercise="${index}" class="${index===state.flipIndex?"active":""}">${index+1}</button>`).join("")}</div><button type="button" data-action="next-exercise">下一题</button></div><article class="flip-card ${back ? "show-back" : ""}" tabindex="0" data-action="flip-card" aria-label="${back ? "答案面" : "题目面"}，按 Enter 或空格翻面"><section class="flip-front"><header><span>题目 ${state.flipIndex+1}</span>${statusPill(item.status)}</header><h2>${escapeHtml(item.question)}</h2><div class="point-row">${item.points.map((point)=>`<span>${escapeHtml(point)}</span>`).join("")}</div><button type="button" data-action="flip-card">查看答案</button></section><section class="flip-back"><header><span>答案与解析</span>${statusPill(item.status)}</header><h3>正确答案</h3><p>${escapeHtml(item.answer)}</p><h3>解析</h3><p>${escapeHtml(item.solution)}</p><div class="training-focus"><strong>训练重点</strong>${item.points.map((point)=>`<span>${escapeHtml(point)}</span>`).join("")}</div><div class="status-controls" data-no-flip><span>标记掌握状态</span><button type="button" data-set-status="doubt" aria-pressed="${item.status === "doubt"}">有疑问</button><button type="button" data-set-status="mastered" aria-pressed="${item.status === "mastered"}">已掌握</button></div><footer data-no-flip><button type="button" data-action="flip-card">返回题目</button><div>${action("交给 Agent 修改", "agent-edit-practice")}${action("手工编辑", "edit-practice", "primary")}</div></footer></section></article>`;
}
function statusFilter(state) {
  const filtered = exercises.filter((item)=>state.filterStatus === "all" || item.status === state.filterStatus);
  return `<div class="filter-hero"><div><span class="section-kicker">GLOBAL REVIEW</span><h2>跨批次按状态筛选</h2><p>批次只保留轻量分隔，练习题本身是视觉主体。</p></div><div class="status-filter">${Object.entries(statusText).map(([key,label])=>`<button type="button" data-status-filter="${key}" aria-pressed="${state.filterStatus===key}">${label}</button>`).join("")}</div></div><div class="weak-group"><small>今天 · 导数边界与符号表强化</small>${filtered.map((item,index)=>`<button type="button" data-filtered-review="${item.id}"><span>${index+1}</span><p>${escapeHtml(item.question)}</p>${statusPill(item.status)}</button>`).join("") || "<p class='empty-note'>当前筛选下没有练习。</p>"}</div>`;
}
function editor(state) {
  const item = exercises[state.flipIndex];
  return `<form class="editor-sheet"><div class="editor-tabs"><strong>练习卡 ${item.id}</strong><span>保存后维持“${statusText[item.status]}”状态</span></div><label class="body-editor">题目<textarea>${escapeHtml(item.question)}</textarea></label><label class="body-editor">正确答案<textarea>${escapeHtml(item.answer)}</textarea></label><label class="body-editor">解析<textarea>${escapeHtml(item.solution)}</textarea></label><div class="point-manager"><div><strong>关联知识点（至少 1 个）</strong><small>只能选择已有知识点，禁止在练习流程中新建。</small></div><div>${item.points.map((point)=>`<span>${escapeHtml(point)} ×</span>`).join("")}<button type="button">选择已有知识点</button></div></div><div class="editor-foot"><span>本地校验：必填、类型、长度、公式可渲染、ID 与批内精确去重。</span>${action("保存 revision", "save-practice", "primary")}</div></form>`;
}
