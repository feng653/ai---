...
function detail(id) {
  ...
  return `<article class="knowledge-detail"><header><div><span class="type-chip">${escapeHtml(card.type)}</span><span class="revision">revision 4 · ${card.updated}</span></div><div class="row-actions">${action("返回列表", "knowledge-list")}${action("编辑", "edit-knowledge", "primary")}</div></header><h2>${escapeHtml(card.title)}</h2><div class="point-row"><strong>关联知识点</strong>${points}</div><div class="markdown-body">${body}</div><aside class="inline-guidance"><strong>需要 AI 继续处理？</strong><span>请打开右下角唯一的 AI Agent，在其中选择“生成或完善知识卡”或“生成练习”。</span></aside><footer><span>关联 ${card.points.length} 个知识点 · 组织关系不等于 AI 上下文</span>${action("删除此卡", "delete-knowledge", "danger")}</footer></article>`;
}
function editor(id) {
  const card = knowledgeCards.find((item) => item.id === id);
  const points = (card?.points || []).map((point) => `<span>${escapeHtml(point)} ×</span>`).join("");
  return `<form class="editor-sheet"><div class="editor-top"><label>标题 <input value="${escapeHtml(card?.title || "")}" required placeholder="必填"></label><label>类型（可选）<select><option>${escapeHtml(card?.type || "未分类")}</option><option>概念</option><option>方法</option><option>易错细节</option><option>+ 新建自定义类型</option></select></label></div><div class="point-manager"><div><strong>关联知识点</strong><small>可多选、移除；手工编辑知识卡时可新建知识点。</small></div><div>${points}<button type="button" data-action="manage-points">管理知识点</button></div></div><div class="editor-tabs"><button class="active" type="button">编辑</button><button type="button" data-action="preview-editor">预览</button><span>草稿已自动保存</span></div><label class="body-editor">正文（可留空）<textarea placeholder="支持 Markdown 与公式">${escapeHtml(card?.body || "")}</textarea></label><div class="editor-foot"><span>正文为空时保存为“待补充”，不能用于生成练习。</span><div class="row-actions">${action("取消并返回", card ? "knowledge-detail" : "knowledge-list")}${action("保存并创建 revision", "save-knowledge", "primary")}</div></div></form>`;
}
