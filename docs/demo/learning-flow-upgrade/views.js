(() => {
  ...
  function list() {
    ...
    return `<div class="page-head"><div><h1>${type === 'mistake' ? '我的错题' : '错因复习'}</h1></div>${type === 'mistake' ? button(draft ? '继续编辑' : '添加错题', draft ? 'edit' : 'new', draft?.id || '', 'primary') : type === 'practice' ? button('生成复习问答', 'build', '', 'primary') : ''}</div>
