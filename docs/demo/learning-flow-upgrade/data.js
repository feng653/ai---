(() => {
  ...
  const names = { mistake: '错题', practice: '复习问答' };
  function initial() {
    return {
  ...
  state.cards = state.cards.filter(item => item.type === 'mistake' || item.type === 'practice');
  state.drafts = Object.fromEntries(Object.entries(state.drafts).filter(([, item]) => item.type === 'mistake' || item.type === 'practice'));
  if (!names[state.page]) { state.page = 'mistake'; state.view = 'list'; }
  if (state.proposal && !names[state.proposal.after?.type]) state.proposal = null;
  state.proposalQueue = (state.proposalQueue || []).filter(item => names[item.after?.type]);
  state.treePath ||= [];
